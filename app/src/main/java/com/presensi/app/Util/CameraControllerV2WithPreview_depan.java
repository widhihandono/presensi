...
import com.presensi.app.Presence_Activity;
...
public class CameraControllerV2WithPreview_depan {
    ...
    private boolean id_camera;


    public CameraControllerV2WithPreview_depan(Activity activity, TextureView textureView, boolean id_camera) {
                    ...
                    else
                    {
                        ...
                        if (afState == null) {
                            captureStillPicture();
                                ...
                                CaptureResult.CONTROL_AF_STATE_NOT_FOCUSED_LOCKED == afState ||
                                CaptureResult.CONTROL_AF_STATE_INACTIVE == afState /*add this*/) { //take picture from camera
                            // CONTROL_AE_STATE can be null on some devices
                            Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                            if (aeState == null || aeState == CaptureResult.CONTROL_AE_STATE_CONVERGED) {
                                mState = STATE_PICTURE_TAKEN;
...
////                    Toast.makeText(activity.getApplicationContext(),"Kosong",Toast.LENGTH_LONG).show();
////                return ;
////                    captureStillPicture();
//                }
//
...
//                if (map == null) {
//                    Toast.makeText(activity.getApplicationContext(),"Kosong_2",Toast.LENGTH_LONG).show();
//                }
//
...
//            }
    ...
    public void takePicture(String a) {
        ...
        Intent intent = new Intent(activity.getApplicationContext(), Presence_Activity.class);
        intent.putExtra("presensi", a);
        intent.putExtra("imageUri", PhotoProvider.getPhotoUri(new File(file, "cropped")).toString());
        intent.putExtra("currentPhotoPath",file.getAbsolutePath());
